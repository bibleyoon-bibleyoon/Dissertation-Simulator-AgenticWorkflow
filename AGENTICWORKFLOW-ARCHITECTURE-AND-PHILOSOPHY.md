...
graph TD
    ...
    B --> C["자식 1: Dissertation Simulator<br/>211-step 박사 논문 / 58 agents"]
...
graph TB
    subgraph "AgenticWorkflow 코드베이스 (설계 프레임워크)"
        ...
        subgraph "Context Preservation + Safety"
            CPS["hooks/scripts/<br/>67개 프로덕션 + 6개 모듈 + 47개 테스트 스크립트<br/>(Core 4 + pCCS 7 + PD 4 + Quality 22 + Safety 6 + Context 7 + 기타 17)"]
...
AgenticWorkflow의 DNA 유전 철학(§1.4)을 실증하는 최초의 대규모 자식 시스템이다. 211-step 박사 논문 연구 시뮬레이션으로, 부모의 전체 게놈(절대 기준, SOT, 4계층 QA, Safety Hook)이 논문 도메인에 맞게 발현된다.
...
├── Wave 1: Basic Search         (Step 39-54)   ── Gate-1 (55-58) ──▶
├── Wave 2: Deep Analysis        (Step 59-74)   ── Gate-2 (75-78) ──▶
├── Wave 3: Critical Analysis    (Step 79-94)   ── Gate-3 (95-98) ──▶
├── Wave 4: Integrated Synthesis (Step 99-106)
├── SRCS Full Evaluation         (Step 107-110)
├── Wave 5: Quality Assurance    (Step 111-114)
└── HITL-2: Literature Review Approval          (Step 115-120)
...
| 워크플로우 | 211-step (논문 4 Phase) | 20-step (3-Phase) |
